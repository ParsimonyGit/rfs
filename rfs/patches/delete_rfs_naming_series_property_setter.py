...
def execute():
	...
	property_setter_default_name = frappe.db.exists(
		"Property Setter", 
		dict(doc_type="Request For Sample RFS",field_name="naming_series", property="default",value="RFS-.#")
	)
	if property_setter_default_name:    
		frappe.db.sql(
			...
			WHERE `tabProperty Setter`.doc_type='Request For Sample RFS'
				AND `tabProperty Setter`.value='RFS-.#'
	...
	property_setter_options_name = frappe.db.exists(
		"Property Setter", 
		dict(doc_type="Request For Sample RFS",field_name="naming_series", property="options",value="RFS-.#")
	)
